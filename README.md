...
| 版本 | 日期 | 更新内容 |
|---|---|---|
| v2.2.0 | 2026-06-27 | **Bug 修复**：TTS 音频解码失败（Edge TTS 代理失效改为 WebSocket 直连）、Markdown 渲染器 const 作用域、角色卡导出 Base64 兼容性、群聊选择计数模板兼容、编辑助手消息持久化、服务端流式请求 HTTP 200 误报为错误 |
| v2.1.0 | 2026-06-26 | 图片渲染、触觉反馈、群聊/多角色支持 |
| v2.0.0 | 2026-06-26 | **大版本升级**：流式输出、重新生成/编辑消息、Markdown 渲染、角色卡导出、头像跨会话缓存、直连模式聊天本地持久化、离线检测、安全加固 |
| v1.3.4 | 2026-06-20 | 安全修复：直连 API Key 不再明文写入本地存储；移除泄露 Key 长度的 debug 日志 |
| v1.3.3 | 2026-06-20 | 修复发送消息后键盘自动收起：清空输入后立即恢复焦点，保持键盘打开 |
| v1.3.2 | 2026-06-20 | 优化聊天滚动体验：发送/回复后自动滚到底部，无需手动下滑 |
| v1.3.1 | 2026-06-20 | 修复空 Key 覆盖 secrets + proxy_password 为空 |
...
| 角色列表 | ✅ | 浏览、搜索、头像异步下载显示（跨会话缓存） |
| AI 对话 | ✅ | 流式输出 + 非流式双模式，支持停止生成 |
| 聊天历史 | ✅ | 服务端自动同步 + 直连模式本地持久化 |
| 重新生成 | ✅ | 重新生成最后一条 AI 回复 |
| 编辑消息 | ✅ | 长按消息可编辑、复制、删除 |
| Markdown 渲染 | ✅ | 助手消息支持粗体、斜体、代码块、引用、删除线、图片 |
| 图片渲染 | ✅ | 支持 `![alt](url)` 和裸图片 URL，点击全屏预览 |
| 触觉反馈 | ✅ | 发送、长按、按钮等交互场景振动反馈 |
| TTS 语音朗读 | ✅ | 长按消息朗读，多音色选择，AI 情感分析自动调整语气 |
| 群聊/多角色 | ✅ | 多角色群聊，轮流回复 + @提及指定回复 |
| 创建角色 | ✅ | 头像选择 + 表单填写 + multipart 上传 |
| 编辑角色 | ✅ | 修改角色设定、人设、开场白等 |
| 导入角色 | ✅ | 从文件/相册/聊天记录/URL 导入 PNG/JSON 角色卡 |
| 导出角色 | ✅ | 长按角色卡片导出为 PNG 角色卡（嵌入 JSON）或纯 JSON |
| 删除角色 | ✅ | 长按菜单删除 |
...
│   ├── storage.js                  # 本地存储
│   ├── util.js                     # 通用工具函数
│   └── markdown.js                 # 轻量 Markdown → WXML 转换器
├── services/
